...
// Add sliding panel for points list
const pointsListPanel = document.createElement('div');
pointsListPanel.id = 'points-list-panel';
pointsListPanel.style.display = 'none';
document.body.appendChild(pointsListPanel);

// Add button to toggle the panel
const togglePointsListBtn = document.createElement('button');
...
document.body.appendChild(togglePointsListBtn);

// Event listener to toggle the sliding panel
togglePointsListBtn.addEventListener('click', () => {
    if (pointsListPanel.style.transform === 'translateX(0px)') {
        pointsListPanel.style.transform = 'translateX(-250px)';
        pointsListPanel.style.transition = 'transform 0.3s ease-in-out';
        togglePointsListBtn.textContent = 'Show Points List';
    } else {
        pointsListPanel.style.transform = 'translateX(0px)';
        pointsListPanel.style.transition = 'transform 0.3s ease-in-out';
...
function updatePointsList(pins) {
    pointsListPanel.innerHTML = '<h3>Points List</h3>'; // Add a header
    const pointsArray = Object.entries(pins).sort((a, b) => b[1].timestamp - a[1].timestamp); // Sort by timestamp
    ...
    pointsArray.forEach(([id, pin]) => {
        ...
        // Add hover effect to show snapshot
        const snapshot = document.createElement('iframe');
        snapshot.src = pin.link;
        ...
        pointsListPanel.appendChild(listItem);
        pointsListPanel.appendChild(snapshot);
    });
}
...
database.ref('pins').on('value', (snapshot) => {
    const pins = snapshot.val();
    if (pins) {
        updatePointsList(pins);
    } else {
        pointsListPanel.innerHTML = '<h3>No points added yet.</h3>';
    }
});
